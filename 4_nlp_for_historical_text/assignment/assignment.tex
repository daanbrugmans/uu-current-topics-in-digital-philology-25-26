...
\begin{enumerate}
    ...
    \end{enumerate}
    \item To my understanding, none of the rules are dependant upon one another, neither intentionally nor unintentionally. The result of running any of the rules before another one should not change the resulting text, as none of the patterns for the rule inputs match any of the patterns for the rule outputs.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Levenshtein-based normalization applies normalization by comparing the edit distances between the historical and modern spellings of a word. It has a bunch of advantages: it is language-independent, it doesn't require any manual work by humans, and we don't need any data for training a model to do the normalization. However, it does require that there exists a robust corpus of the modern version of the language in which text is normalized, which is not always the case. When such a robust enough dictionary doesn't exist, Levenshtein-based normalization isn't feasible.
\end{enumerate}

\subsection{4}
\begin{enumerate}
    \item According to my evaluation script, 74.5\% of the tokens share the same spelling between the original, unaltered historical text and the ground truth modern English text.
    \item According to my evaluation script, 74.8\% of the tokens share the same spelling between the historical text after the rules have been applied and the ground truth modern English text.
    \item My rules have introduced only very slight improvements. I feel that the base accuracy is already quite high, and when inspecting my implementation during debugging, it seems that the score is achieved by many words already being the same across both the historical and modern variants. As I expected, there were some incorrect changes made by my rules caused by false positives: \begin{enumerate}
        \item In the sentence starting with ``It was in the Elizabeth Bonaventure [\dots]'', ``Elizabeth'' was changed to ``Elizabe'' due to my ``-eth'' rule. This is simply not correct. One way this may have been avoided is by not applying the rule to proper nouns.
        \item In the sentence starting with ``the K. of Poland is in Silesia hunts and passeth [\dots]'', ``passeth'' is changed to ``passe''. This is how I expected the implementation of my rule to act, but it is not correct English and should be ``passes''. This could've been avoided by making the ``-eth'' rule more specific, or by instead using a check that looks at which verb conjugation would match the noun preceding it.
        \item In the sentence ``I know you will not think it fit my Lord should discharge an Officer of the Field but in a regulate way'', ``discharge'' is changed to ``disharge'' due to my ``sch'' rule. This is unintentional and an edge case I did not consider. One way this could've been avoided by checking if the word containing the ``sch'' is actually a compound word, which is true in this case with ``dis-'' and ``charge''.
    \end{enumerate}
\end{enumerate}
...
\begin{itemize}
    \item ``wyth'' marked as a base form verb (VB) changed to ``with'' marked as a preposition (IN). This is an example of my rule for changing ``y(consonant)'' to ``(i)consonant'' working as intended.
    \item ``durying'' marked as a base form verb (VB) changed to ``during'' marked as a preposition (IN). This is another example of my rule for changing ``y(consonant)'' to ``(i)consonant'' working as intended.
    \item ``myself'' marked as a personal pronoun changing to ``miself'' marked as a singular or mass noun (NN). This is an example of my rule for changing ``y(consonant)'' to ``(i)consonant'' NOT working as intended, and introducing new mistakes.
    \item ``anie'' marked as a singular or mass noun changing to ``any'' marked as a determiner (DT). This is an example of my ``-ie'' to ``-y'' rule working as intended.
    \item ``teeth'' marked as a plural noun (NNS) changing into ``tee'' marked as a singular or mass noun. This is an example of my ``-eth'' to ``-e'' rule introducing a significant error, as it not only falsely changes the Part-of-Speech, but also the semantics of the word.
\end{itemize}
...
\section{Subtask 3: Historical Cryptography}
\subsection{1}
\begin{enumerate}
    \item Transposition is a straightforward encryption method where the letters of the unaltered plaintext are put in a different order. This produces words that use the same letters as the original word, but in a different order. For example, we could transpose the word ``charisma'' into ``amsriach''.
    \item Substitution is an encryption method where the letters of the unaltered plaintext are replaced by other symbols. A simple way of doing this is by shifting the letters of the alphabet used in the plaintext $X$ amount of times. The ciphertext can then be decoded by shifting the alphabet by the same amount in the other direction. As a simple example, if we shifted the alphabet forward by one letter, with ``A'' becoming ``B'', ``B'' becoming ``C'', etc., we could encode the word ``christmas'' as ``disjtunbt''. Deciphering the text is as simple as shifting the letters back by one position.
\end{enumerate}
